...
function buildNestedReducer(path, reducer) {
  return path
    .split('.')
    .reverse()
    .reduce(
      (nestedReducer, key) => combineReducers({ [key]: nestedReducer }),
      reducer
    );
}

export function createTestStore(path, reducer, middlewares = []) {
  const store = _createStore(
    buildNestedReducer(path, reducer),
